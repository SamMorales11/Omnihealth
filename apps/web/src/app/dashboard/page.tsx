    ...
    <main className="p-8 md:p-12 w-full min-h-screen bg-slate-50 dark:bg-transparent transition-colors duration-500">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
          ...
          {/* MENU NAVIGASI KANAN (Diperbaiki Kontrasnya) */}
          <div className="flex gap-4 bg-white dark:bg-slate-800/80 backdrop-blur-md p-1.5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm items-center transition-colors duration-500">
            <Link href="/doctors" className="px-5 py-2 text-sm font-bold text-slate-600 dark:text-slate-300 hover:text-teal-600 dark:hover:text-teal-400 hover:bg-teal-50 dark:hover:bg-slate-700 rounded-lg transition-all">Data Dokter</Link>
            <Link href="/appointments" className="px-5 py-2 text-sm font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 bg-indigo-50 dark:bg-indigo-500/10 rounded-lg transition-all">Jadwal Antrean ➔</Link>
            <div className="w-px h-6 bg-slate-200 dark:bg-slate-700 mx-1 transition-colors duration-500"></div> 
            <LogoutButton />
          </div>
        </div>

        {/* KARTU STATISTIK (Diperbaiki jadi bg-slate-800 agar menonjol) */}
        ...
        {/* GRAFIK (Wajib direvisi di file terpisah) */}
        <DashboardCharts appointments={appointments} />

        {/* TABEL DIREKTORI (Diperbaiki jadi bg-slate-800) */}
