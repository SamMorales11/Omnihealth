      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Manajemen Antrean</h1>
        ...
        <form onSubmit={handleAdd} className="bg-white p-6 md:p-8 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 mb-8 flex flex-col md:flex-row gap-6 items-end relative overflow-hidden">
          ...
          <div className="flex-1 w-full">
            <label className="text-xs font-bold text-slate-500 mb-2 block uppercase tracking-wider">Pilih Pasien</label>
            <div className="relative">
              <select required value={patientId} onChange={e => setPatientId(e.target.value)} className="w-full appearance-none bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 block p-3 transition-all cursor-pointer">
          ...
          <div className="flex-1 w-full">
            <label className="text-xs font-bold text-slate-500 mb-2 block uppercase tracking-wider">Dokter Pemeriksa</label>
            <select required value={doctorId} onChange={e => setDoctorId(e.target.value)} className="w-full appearance-none bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 block p-3 transition-all cursor-pointer">
          ...
          <div className="flex-1 w-full">
            <label className="text-xs font-bold text-slate-500 mb-2 block uppercase tracking-wider">Tanggal</label>
            <input required type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 block p-3 transition-all cursor-pointer" />
          </div>
          
          <button type="submit" disabled={isSubmitting} className="w-full md:w-auto bg-indigo-600 text-white px-8 py-3 rounded-xl font-semibold hover:bg-indigo-700 hover:shadow-lg hover:shadow-indigo-200 transition-all disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2">
            {isSubmitting ? (
            ...
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-slate-50/80 backdrop-blur-sm">
                <tr>
                  ...
                  <th className="px-8 py-5 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Pasien</th>
                  <th className="px-8 py-5 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Dokter Pemeriksa</th>
              ...
              <tbody className="bg-white divide-y divide-slate-100">
                {appointments.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-8 py-12 text-center text-slate-500 font-medium">Belum ada jadwal antrean terdaftar.</td>
                  </tr>
                ) : appointments.map(a => (
                  <tr key={a.id} className="hover:bg-slate-50/80 transition-colors group">
                    ...
                    <td className="px-8 py-5 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-bold text-slate-900">{a.patientName}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-8 py-5 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {a.specialist && <p className="text-xs font-medium text-slate-500 mt-0.5">{a.specialist}</p>}
                        </div>
                      </div>
                    </td>

                    <td className="px-8 py-5 whitespace-nowrap">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold border shadow-sm ${getStatusBadge(a.status)}`}>
                    ...
                    <td className="px-8 py-5 text-right whitespace-nowrap">
                      {a.status === 'Menunggu' ? (
                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                          <button onClick={() => handleUpdateStatus(a.id, 'Selesai')} className="px-3 py-1.5 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 rounded-lg text-xs font-bold transition-colors">Selesai</button>
                          <button onClick={() => handleUpdateStatus(a.id, 'Dibatalkan')} className="px-3 py-1.5 bg-rose-50 text-rose-700 hover:bg-rose-100 rounded-lg text-xs font-bold transition-colors">Batal</button>
                        </div>
                      ) : (
                        <span className="text-slate-400 italic text-xs font-medium">Terkunci</span>
                      )}
                    </td>
                  </tr>
                ))}
